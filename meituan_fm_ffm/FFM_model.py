...
def inference(input_x, input_x_field, zeroWeights, oneDimWeights, thirdWeight):
    ...
    for feature_i in range(input_shape):
        featureIndex1 = feature_i
        fieldIndex1 = int(input_x_field[feature_i])
        for feature_j in range(feature_i + 1, input_shape):
            featureIndex2 = feature_j
            fieldIndex2 = int(input_x_field[feature_j])
...
def gen_data():
    labels = [-1,1]
    train_y = [np.random.choice(labels,1)[0] for _ in range(all_data_size)]
    train_x_field = [i // 10 for i in range(input_x_size)]
    train_x = np.random.randint(0,2, size=(all_data_size, input_x_size))
...
if __name__ == '__main__':
    ...
    #创建二次项的cross feature 权重变量, size = n * f * k
    thirdWeight = createTwoDimensionWeight(input_x_size, field_size, vector_dimension)

    y_ = inference(input_x, train_x_field, zeroWeights, oneDimWeights, thirdWeight)
    ...
    train_step = tf.train.GradientDescentOptimizer(learning_rate=lr).minimize(loss, global_step=global_step)
